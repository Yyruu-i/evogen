...
async def _llm_stream_generator(message: str, session_id: str, user_id: str = "default"):
    ...
    system_prompt += (
        ...
        "\\n"
        "### 规则3：全自动编排（一句话触发完整流程）\\n"
        "用户说「检查 192.168.1.1 的安全性」时，你应该自动执行完整5步流程：\\n"
        "步骤1 → `ping_sweep(\"192.168.1.1\")` 或直接 `security_scan(\"192.168.1.1\")`（如果明确指定IP则跳步骤1）\\n"
        "步骤2 → `security_scan(\"192.168.1.1\", vendor=\"绿盟,天融信,安恒\")`（TOP500端口+服务识别+风险分级）\\n"
        "步骤3 → `baseline_check(\"192.168.1.1\")`（等保基线核查）\\n"
        "步骤4 → `evidence_snapshot(\"192.168.1.1\")`（证据固化）\\n"
        "步骤5 → `gen_security_report(\"192.168.1.1\", template=\"standard\")`（自动选模板出报告）\\n"
        ...
        "- `smart_orchestrator(target, scan_type)`: 旧编排器（端口+漏洞+Rootkit）\\n"
        "- `port_scan(target)`: 旧端口扫描（仅5个端口，**不如 security_scan**）\\n"
        "- `vuln_scan(target)`: 旧漏洞扫描（Nuclei）\\n"
        "- `rkhunter_scan(check_all?)`: Rootkit检测\\n"
        "- `chkrootkit_scan(quick?)`: chkrootkit检测\\n"
