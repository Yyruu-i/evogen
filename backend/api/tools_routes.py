...
def _static_tool_list() -> list[dict]:
    ...
    return [
        ...
        {"name": "ping_sweep", "description": "ICMP存活探测 — 扫描目标网段发现存活主机，用于检测前的资产摸底阶段", "toolset": "security", "requires_env": [], "parameters": [{"name": "subnet", "type": "string", "description": "目标网段（如 192.168.1.0/24）", "required": True}]},
        {"name": "security_scan", "description": "安全检查 — 对单个IP执行端口扫描+风险分析，输出风险等级和修复建议", "toolset": "security", "requires_env": [], "parameters": [{"name": "target", "type": "string", "description": "目标IP地址", "required": True}, {"name": "vendor", "type": "string", "description": "厂商名称（可选）", "required": False}, {"name": "scan_type", "type": "string", "description": "扫描类型: port_scan/full", "required": False}]},
        {"name": "batch_scan", "description": "批量安全扫描 — 对多个目标批量执行安全检查，自动汇总结果和风险统计", "toolset": "security", "requires_env": [], "parameters": [{"name": "targets", "type": "array", "description": "目标IP列表", "required": True}, {"name": "scan_type", "type": "string", "description": "扫描类型", "required": False}, {"name": "vendor", "type": "string", "description": "厂商名称", "required": False}]},
        {"name": "gen_security_report", "description": "生成安全检查报告 — 按模板生成Markdown格式的结构化安全检测报告", "toolset": "security", "requires_env": [], "parameters": [{"name": "target", "type": "string", "description": "目标IP", "required": True}, {"name": "template", "type": "string", "description": "模板: standard/customer", "required": False}]},
